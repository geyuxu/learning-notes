...
description: '在电商搜索系统中，Cassandra数据库中累积了海量的商品数据，经过长时间运营，其中累积了大量已下架或删除的无效商品记录。这些无效数据不仅占用存储空间，还可能影响搜索结果的准确性。因此，我们需要对Cassandra中的历史商品数据进行清理。而待清理的数据规模巨大，这给清理工作带来了不小的挑战。'
series: 'Cassandra专题'
pubDate: '2022/02/24'
---
...
在电商搜索系统中，Cassandra数据库中累积了海量的商品数据，经过长时间运营，其中累积了大量已下架或删除的无效商品记录。这些无效数据不仅占用存储空间，还可能影响搜索结果的准确性。因此，我们需要对Cassandra中的历史商品数据进行清理。而待清理的数据规模巨大，这给清理工作带来了不小的挑战。
