...
description: "系统性梳理分类模型的评估指标，覆盖从混淆矩阵、准确率、精确率、召回率到AUC、LogLoss、MCC等核心指标。本文深入探讨了各指标的适用场景、常见误区，并提供了处理多分类和不均衡数据的策略及Python实战代码模板。"
pubDate: "2025-07-26 20:14:48"
heroImage: "/placeholder.webp"
tags: ["Machine Learning", "Classification", "Metrics", "Python"]
---

与回归模型不同，分类模型的评估更为复杂，因为它不仅要看“预测对不对”，还要看“怎么对，怎么错”。一个看似准确率很高的模型，在面对不均衡数据时可能毫无价值。因此，掌握一套全面、立体的评估指标体系至关重要。

本文将按照“理解混淆矩阵 → 常用点值指标 → 阈值曲线指标 → 概率校准指标 → 多分类／不均衡处理 → 实战代码模板”的脉络，为你系统梳理分类模型评估的全景，并指出常见误区与使用建议。

### 1. 混淆矩阵：所有指标的“母体”

混淆矩阵（Confusion Matrix）是衡量分类模型性能的基础，它将样本的真实类别与模型的预测类别进行比较，构成一个二维表格。
