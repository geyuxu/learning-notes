...
description: "本文深入探讨了机器学习中超参数调优的核心概念、主流方法与实战技巧。从网格搜索、随机搜索到贝叶斯优化与最新自动化工具，我们不仅剖析了每种技术的原理与适用场景，还提供了基于 Scikit-learn, PyTorch, Optuna, Ray Tune 的可运行代码示例，助你高效提升模型性能。"
...
超参数是训练前就要决定的“外部”参数；如何高效搜索这些参数的组合、并理解它们对收敛速度与泛化能力的影响，是提升模型性能最具性价比的手段之一。

## 1. 什么是超参数？

在机器学习中，模型参数和超参数是两个核心但又截然不同的概念。

| 概念 | 典型示例 | 与“参数”区别 |
| :--- | :--- | :--- |
| 训练前人工设定的数值，左右训练过程本身，却不在反向传播中更新 | 学习率 (lr)、批大小 (batch_size)、模型深度、正则强度 λ、Dropout 比例等 | **参数（weights）**在训练中被梯度更新；超参数只能通过外部搜索 / 经验手动调整 |

简而言之，**参数**是模型在训练过程中自动学习的，例如神经网络的权重和偏置。而**超参数**则是在训练开始之前由我们手动设置的，它们控制着学习过程的行为和性能。
