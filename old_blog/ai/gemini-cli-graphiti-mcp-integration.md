...
description: "本文详细介绍了如何将 Gemini CLI 与 Graphiti MCP (Memory, Control, Perception) 进行对接，通过两种核心方式（本地 stdio 和远程 SSE）实现 Gemini 对 Graphiti 强大工具集的调用，包括记忆、检索和知识图谱等功能。文章涵盖了环境准备、详细配置、常见问题排查及一键式部署脚本，旨在帮助开发者快速打通两大工具，构建更智能的 CLI 工作流。"
pubDate: "Jul 25 2025"
...
1.  **本地 stdio 方式**：Gemini CLI 在启动时，会自动 `fork` 一个 Graphiti 子进程，并通过标准输入/输出（`stdin`/`stdout`）进行高效的本地通信。这种方式配置简单，无需网络设置。
2.  **远端 HTTP/SSE 方式**：Graphiti 作为一个常驻服务（通过 Docker 或源码 `uv run` 启动），Gemini CLI 则通过 Server-Sent Events (SSE) 长连接来访问它。这种方式更适合分布式或服务化的部署场景。
