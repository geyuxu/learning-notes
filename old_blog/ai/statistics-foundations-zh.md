...
title: "统计推断基础：从数据到决策"
pubDate: 2025-01-24
description: "探索统计学的核心原则，从伯努利模型到假设检验，理解如何从数据中获得有意义的洞察。"
---

# 统计推断基础：从数据到决策

统计学是从数据中学习的科学。其核心在于提供一个思考世界的框架：我们如何提出问题，需要什么数据来回答这些问题，以及我们能够自信地得出什么结论？这篇文章受到 MIT 18.6501x 课程基础概念的启发，探索了指导我们从原始数据到有意义洞察的核心原则。

## 统计问题的核心框架

要理解统计学，我们必须首先区分它与概率的不同。概率学从已知的真相出发，预测数据。例如，如果我们知道一个骰子是公平的，我们可以计算掷出六的概率。这是一个演绎过程。

统计学的工作方向相反。我们从观察到的数据开始，试图推断潜在的真相。我们有骰子投掷的结果，但不知道骰子是否公平。我们的目标是利用数据对这个真相做出有根据的推测。这是一个归纳过程。
...
![典型的原始显微图：成千上万的蛋白质颗粒被埋在随机噪声的海洋中。](https://img.geyuxu.com/2025-05-27-133802HS2DtK.png)

为了重构结构，图像被对齐然后平均。因为每个图像中的噪声是随机和独立的，它在平均过程中相互抵消。然而，蛋白质的微弱潜在信号是一致的，并在平均中得到强化。

![对齐后的 2D 类别平均。微弱的颗粒，当平均后，显示出清晰的环状结构。](https://img.geyuxu.com/2025-05-27-134144UY3L1Y.png)
...
![Cryo-EM 工作流程：对齐 → 平均 → 迭代 → 重构 3D 图。](https://img.geyuxu.com/2025-05-27-134219I9Pul9.png)

结果是分子的高分辨率 3D 图，这一成就因统计学平均的基本原理而成为可能。

![最终 3D 重构的切片，密度按强度着色。](https://img.geyuxu.com/2025-05-27-134257qN7BvW.png)
...
![接吻研究的模拟结果](https://img.geyuxu.com/2025-05-27-141307jig87U.png)
